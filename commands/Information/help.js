    ...
    try {
      if (args[0]) {
        ...
        const helpEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: '-win', value: 'Add a win for one or more users. If no users are mentioned, adds a win for the message author.' },
            { name: '-removewin', value: 'Remove a win for a specific user.' },
            { name: '-leaderboard', value: 'Show the top 5 users with the most wins.' },
            { name: '-resetAllWins', value: 'Reset all win totals to 0.' },
          )
          .setTimestamp();
